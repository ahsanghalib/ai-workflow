...
The provider/plugin command examples in this reference are optional adapters,
not requirements of the shared skill. Run only checks supported by the active
harness; never install dependencies automatically. If a capability is absent,
report it as unavailable and continue with the source-only workflow.
