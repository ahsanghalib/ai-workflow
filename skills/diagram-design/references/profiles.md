...
Profile paths are a portable convention, not a requirement to write outside
the authorized workspace. Prefer a project-local profile location when the
active harness cannot safely access the user's home directory, and report the
chosen location. Provider-specific install paths mentioned elsewhere are
optional adapters.
