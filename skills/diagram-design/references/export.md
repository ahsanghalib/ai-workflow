...
This is an optional capability path. Use the active harness's approved local
renderer when one is available; do not assume a plugin slash command, global
Python package, browser cache, or network access. If none is available, stop at
the HTML/SVG source and report the limitation.
...
3. Make it standalone:
   ...
   - Optionally inject Google Fonts `@import` so the SVG renders with the
     preferred typography in a browser. **XML-escape the `&` separators as
     `&amp;`** — a standalone `.svg` is parsed as strict XML, where a bare `&`
     starts an entity reference and makes the whole file fail to parse. Keep
     the local/system font fallback when remote fonts are unavailable.
     ```svg
     <defs>
...
If the import fails, surface the missing-capability instruction to the user and
stop:

> Playwright isn't installed. To enable PNG export, install it in an
> explicitly approved project-local environment, then install a local browser:
> ```
