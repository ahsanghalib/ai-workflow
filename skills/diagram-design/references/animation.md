...
Run the bundled checker from the resolved Diagram Design skill directory:

```bash
python3 scripts/self_check.py path/to/animated-diagram.html
```

The upstream maintainer checkout may provide additional motion and skin
verifiers, but they are not bundled with this portable skill. Run them only
when the files exist in the resolved installation; otherwise report those
specialized checks as unverified and complete the manual checks below.
