...
- Do not discover customer problems, market demand, or product requirements;
  hand unknown inputs to `product-discovery` when that companion skill is
  available.
...
Inspect only the code, tests, configuration structure, documentation, and
history needed to ground the proposal. Never read or reproduce `.env` files,
...
observed constraints from assumptions and inference.
